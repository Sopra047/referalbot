...
def start(message):
   try:
    ...
    if msg == '/start':
        ...
        msg_start = "*🍔 Pour utiliser ce bot, vous devez rejoindre cette chaîne en cliquant sur ce lien 👉 - si vous avez rejoint le canal cliquez sur ✅ Verifier\n\n "
        for i in CHANNELS:
            msg_start += f"\n➡️ {i}\n"
        msg_start += "*"
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Définir le portefeuille":
        ...
        send = bot.send_message(message.chat.id, "_⚠️Envoyez votre adresse de Compte MTN valide Exemple: +237670086719._",
